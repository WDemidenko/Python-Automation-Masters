...
def send_notification(ad_id, message: str = ""):
    ...
    with create_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT name, url, price FROM ads WHERE id_auto=?", (ad_id,))
        name, url, price = cursor.fetchone()
    ...
    for index, link in enumerate(photo_links):
        ...
        with open(file_name, 'rb') as file:
            media.append(telebot.types.InputMediaPhoto(file.read(), caption=full_message if index == 0 else '', parse_mode="HTML"))
